...
-- This creates the channel tables needed for multi-channel support
...
DROP POLICY IF EXISTS "Users can manage own channel connections" ON channel_connections;
...
DROP POLICY IF EXISTS "Users can view own channel sessions" ON channel_sessions;
CREATE POLICY "Users can view own channel sessions"
  ON channel_sessions FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own channel sessions" ON channel_sessions;
CREATE POLICY "Users can insert own channel sessions"
  ON channel_sessions FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own channel sessions" ON channel_sessions;
CREATE POLICY "Users can update own channel sessions"
  ON channel_sessions FOR UPDATE
  USING (auth.uid() = user_id);

-- RLS Policies for channel_messages
DROP POLICY IF EXISTS "Users can view own channel messages" ON channel_messages;
...
DROP POLICY IF EXISTS "Users can insert own channel messages" ON channel_messages;
